...
class RemoteContactGetRequest extends RemoteToolsRequest {
  public const BEFORE_ADD_PROFILE_DATA   = RemoteToolsRequest::EXECUTE_REQUEST + 300;
  public const ADD_PROFILE_DATA          = RemoteToolsRequest::EXECUTE_REQUEST + 250;
    ...
    if (!$request->hasErrors()) {
      try {
        ...
        if (!$profile) {
          ...
          // set default reply to result
          $request->reply = $request->result;
          if (!empty($log_debug)) {
