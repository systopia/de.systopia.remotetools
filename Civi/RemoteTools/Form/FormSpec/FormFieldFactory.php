...
final class FormFieldFactory implements FormFieldFactoryInterface {
  ...
  private const FILE_URL_LIFETIME_HOURS = 1;
  ...
  // phpcs:enable
    ...
    switch ($field['data_type']) {
      ...
      case 'Integer':
        // @todo Handle input_type "EntityRef" and input_type NULL with fk_entity set.
  ...
  private function getFilename(int $fileId): string {
    /** @var string $filename */
    $filename = $this->api4->execute(File::getEntityName(), 'get', [
      'select' => ['file_name'],
      'where' => [['id', '=', $fileId]],
    ])->single()['file_name'];
  ...
  private function setDefaultValue(AbstractFormField $formField, array $field, ?array $entityValues): void {
    if (NULL !== $entityValues && array_key_exists($field['name'], $entityValues)) {
      ...
      if ($formField instanceof FileField && is_int($entityValues[$field['name']])) {
        $formField->setUrl($this->fileUrlGenerator->generateUrl(
            $entityValues[$field['name']],
          self::FILE_URL_LIFETIME_HOURS,
          // @phpstan-ignore argument.type
          $entityValues['id'] ?? NULL
        ));

        $formField->setFilename(
          // @phpstan-ignore argument.type
          $entityValues[$field['name'] . '.file_name'] ?? $this->getFilename($entityValues[$field['name']])
        );
      }
    }
