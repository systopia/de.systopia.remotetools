...
final class FormFieldFactoryTest extends TestCase {
  ...
  private FileUrlGeneratorInterface&MockObject $fileUrlGeneratorMock;
  ...
  protected function setUp(): void {
    ...
    $this->fileUrlGeneratorMock = $this->createMock(FileUrlGeneratorInterface::class);
    $this->settingsMock = $this->createMock(SettingsBag::class);
    $this->factory = new FormFieldFactory(
      $this->api4Mock,
      $this->fileUrlGeneratorMock,
      $this->settingsMock
    );
  }
  ...
  public function testCreateFormFieldFile(): void {
    ...
    $this->api4Mock->expects(static::once())->method('execute')
      ->with('File', 'get', [
        'select' => ['file_name'],
        'where' => [['id', '=', $fileId]],
      ])->willReturn(new Result([['file_name' => 'abc.txt']]));

    $lifeTimeHours = 1;
    $this->fileUrlGeneratorMock->expects(static::once())->method('generateUrl')
      ->with($fileId, $lifeTimeHours, $entityId)
      ->willReturn('https://example.org/def.txt');

    static::assertEquals((new FileField('test', 'test'))
      ->setDefaultValue($fileId)
      ->setFilename('abc.txt')
      ->setUrl('https://example.org/def.txt'),
       $this->factory->createFormField($field, ['test' => $fileId, 'id' => $entityId]));
  }
