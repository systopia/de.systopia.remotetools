...
class CRM_Remotetools_DataTools {
  ...
  public static function setSortingString($sorting_tuples, &$request_data): void {
    ...
    unset($request_data['option.sort'], $request_data['option']['sort']);
    if ($sorting_string) {
  ...
  public static function restrictToIds(&$request, $ids, $field_name = 'id') {
    if (empty($ids)) {
      ...
      elseif (is_array($current_restriction)) {
        ...
        \Civi::log()->debug("DataTools.restrictToIds: couldn't restrict '{$field_name}' parameter: "
          . json_encode($current_restriction));
      }
    }
  }
