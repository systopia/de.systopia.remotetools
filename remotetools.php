 ...
 */
function remotetools_civicrm_config(CRM_Core_Config $config): void {
  ...
  $dispatcher->addUniqueListener(
        'civi.remotecontact.get',
        ['CRM_Remotetools_RemoteContactQueryTools', 'processMultivalueOrSearch'],
        RemoteContactGetRequest::INITIALISATION - 10
  );
  $dispatcher->addUniqueListener(
        'civi.remotecontact.get',
        ['Civi\RemoteContact\RemoteContactGetRequest', 'addProfileRequirements'],
        RemoteContactGetRequest::BEFORE_EXECUTE_REQUEST
  );
  $dispatcher->addUniqueListener(
        'civi.remotecontact.get',
        ['Civi\RemoteContact\RemoteContactGetRequest', 'addProfileRequirements'],
  ...
  $dispatcher->addUniqueListener(
        'civi.remotecontact.get',
        ['Civi\RemoteContact\RemoteContactGetRequest', 'executeRequest'],
        RemoteContactGetRequest::EXECUTE_REQUEST
  );
  $dispatcher->addUniqueListener(
        'civi.remotecontact.get',
        ['Civi\RemoteContact\RemoteContactGetRequest', 'filterResult'],
        RemoteContactGetRequest::AFTER_EXECUTE_REQUEST
  );
...
function remotetools_civicrm_container(ContainerBuilder $container): void {
  ...
  // Deprecated because it's not required with Symfony >=6.2 anymore.
 ...
 */
function remotetools_civicrm_install(): void {
 ...
 */
function remotetools_civicrm_enable(): void {
 ...
 * @param array<string, array<string, list<string>>> $permissions
 */
function remotetools_civicrm_alterAPIPermissions(
  string $entity,
  string $action,
  array &$params,
  array &$permissions
